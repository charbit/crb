...
In this study we are interested by the station design in term of parameter accuracy. In short, in presence of only pure coherent acoustic waves, it is clear that the best design is to locate sensors at far as possible. For which reason could we take into account an upper bound on the aperture ? Regarding the size of the station and the distance of the source, the only effect which can induce a bound on the station aperture, in presence of a coherent acoustic signal, is that the  loss of coherence increases when the distance between sensors increases.
